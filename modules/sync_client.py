...
from config import ERC20_ABI, RPC
...
class Client:
    ...
    def send_transaction(
        ...
        if not max_priority_fee_per_gas:
            ...
            max_priority_fee_per_gas = Client.get_max_priority_fee_per_gas(w3=self.w3, block=last_block)
    ...
    def verif_tx(self, tx_hash) -> bool:
        try:
            ...
            if 'status' in data and data['status'] == 1:
                logger.success(f'{self.address} | transaction was successful: {RPC['polygon']['explorer']}{tx_hash.hex()}')
                return True
            else:
    ...
    def approve_interface(self, token_address: str, spender: str, amount: Optional[TokenAmount] = None) -> bool:
        logger.info(f'{self.address} | approve | start approve USDC for spender')
        decimals = self.get_decimals(contract_address=token_address)
        balance = self.balance_of(contract_address=token_address)

        if balance.Wei <= 0:
            logger.info(f'{self.address} | approve | zero USDC balance')
            return False
        ...
        if not self.verif_tx(tx_hash=tx_hash):
            logger.info(f'{self.address} | approve | USDC for spender')
            return False
        return True
